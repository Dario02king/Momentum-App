...
|---|---|---|---|---|
| 1 | Mental / Gym / Laufen / Ernährung switch | PASS | A | `terminal.mjs`: named radio group, four enabled areas in product order, one checked, one tab stop, arrow keys move the selection; one line at 393/430, two rows of two below 360px, no label clipped |
| 2 | Only one domain visible at a time | PASS | A | `terminal.mjs`: every `[data-metric]` on screen belongs to the selected domain, and only that domain's daily row is rendered — for all four domains |
| 3 | Home global content preserved | PASS | A | `terminal.mjs`: Today still carries the Boss standing, the check-in, the Ernährung rating and both training actions, and no terminal content; `phase6`/`phase7`/`phase8`/`release` unchanged |
| 4 | Global progress preserved | PASS | A | The overall trend and the overall daily row moved to Rang (`terminal.mjs` asserts both are there); Today keeps the Boss summary |
...
| 13 | Accessibility (switch semantics, names, headings) | PASS | A | Switch semantics in `terminal.mjs`; every control and image named, headings in product order, bars named with their number printed beside them, in `phase41-a11y` / `phase5-a11y`. Chart-specific rows are Stage C |
...
| 17 | Browser / Back navigation | PASS | A | `terminal.mjs`: `#/progress/<domain>` and `#/rank` deep links open the right screen; Back and Forward walk the domains visited; an unknown hash keeps the screen and corrects the address, a fresh load of one lands on Today; a switched-off domain is corrected in place |
| 18 | Production build | PASS | A | `tsc -b` clean, `vite build` clean, `release.mjs` 82/82 against the production bundle |
| 19 | Full test suite | PASS | A | `npm run test` 1032 passing, exit 0; every phase suite green except the pre-existing date-dependent `phase41` assertion that fails identically on V1 |
