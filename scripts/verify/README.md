...
| `terminal.mjs` | **The domain terminal (Stage A).** The switch as a named radio group with one tab stop and arrow keys, exactly one domain rendered at a time, Home unchanged, the global trend on Rang, `#/progress/<domain>` deep links, Back and Forward walking the domains visited, a switched-off domain corrected in place, reduced motion |
