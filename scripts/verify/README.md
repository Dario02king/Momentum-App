...
| `safe-area.mjs` | Responsive layout and the iOS safe area: four widths with and without a notched iPhone's insets, plain copy and text the user wrote, horizontal overflow, vertical clipping, that the last card clears the bottom edge, and — read back off the source — that the insets are spent once, by the Screen primitive and the tab bar. What iOS paints *above* the web view it cannot see, and says so |
