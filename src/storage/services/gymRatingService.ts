...
import { enduranceState, type EnduranceState, type EnduranceWeek } from '../../core/gym/endurance';
import {
...
import {
  ...
  type GymRatingDay,
  type GymRatingPoint,
} from '../../core/gym/rating';
import { performanceScore, type PerformanceScore } from '../../core/gym/score';
...
export function performanceOn(days: readonly ExerciseDay[], on: DateKey): WindowPerformance {
  const trendFrom = addDays(on, -(GYM_RATING.TREND_WINDOW_DAYS - 1));
...
export function buildGymRating(input: GymRatingInput): GymRatingState {
  ...
  for (const date of dates) {
    const trendFrom = addDays(date, -(GYM_RATING.TREND_WINDOW_DAYS - 1));
  ...
  const modern = computeGymRating(modernDays, {
